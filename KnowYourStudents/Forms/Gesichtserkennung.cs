...
namespace KnowYourStudents
{
    public partial class Gesichtserkennung : Form
    {
        ...
        private void loadNewStudent()
        {
            ...
            // Note: Order of the students in the list and in the picture-boxes is identical
        ...
        private void evaluatePick(int pictureBoxNumber)
        {
            // Check if timer is currently running, this disables inputs in between games
            if (timeDelayBetweenStudents.Enabled == false)
            {
                // Correct guess    
                if (selectedStudents[pictureBoxNumber] == activeStudent)
                {
                ...
                {
                    lbWinLooseInfo.Text = "Leider falsch! Sie haben auf " + selectedStudents[pictureBoxNumber].Name + " geklickt.";
                    BackColor = Color.Red;
                    timeDelayBetweenStudents.Enabled = true;
                }
            }
